...
if current_data is None:
 st.error("Nuk u mund të merren të dhënat për monedhën e zgjedhur. Provoni përsëri më vonë.")
elif "error" in current_data:
 st.error(f"Gabim API: {current_data['error']}. Kjo mund të jetë për shkak të kufizimeve të normës. Provoni përsëri pas disa minutash.")
else:
...
if not short_term_prices.empty:
 ...
 st.warning("Nuk u gjetën të dhëna afatshkurtra për probabilitete. Kjo mund të jetë për shkak të kufizimeve të API-së.")
...
if historical_prices.empty:
 st.warning("Nuk u gjetën çmime historike. Provoni të rrisni intervalin e kohës ose provoni më vonë.")
else:
 ...
 fig.update_layout(height=1200, title_text=f'Analizë e Plotë për {selected_coin}',
 xaxis_title='Data',
