...
with tab_calc:
    ...
    if price_entry > 0:
        ...
        # ====== NIVELLET STANDARD +1%...+6% & -2% ME 10 DECIMALE ======
        st.markdown("### 📈 Nivellet standarde të çmimit (+1% … +6% dhe −2%) – 10 decimale")
        ...
        for i in range(1, 7):
            ...
            rows_levels.append({
                "Lëvizja": f"+{i}%",
                "Çmimi (10 dec)": f"{price_plus:.10f}"
            })
        # minus 2%
        price_minus2 = price_entry * (1 - 2 / 100.0)
        rows_levels.append({
            "Lëvizja": "-2%",
            "Çmimi (10 dec)": f"{price_minus2:.10f}"
        })

        df_levels = pd.DataFrame(rows_levels)
        st.dataframe(df_levels, use_container_width=True)

        # ====== INPUT MANUAL I PËRQINDJES ======
        st.markdown("### ✍️ Llogarit çmimin sipas përqindjes që TI dëshiron")

        manual_percent = st.number_input(
            "Shkruaj përqindjen (p.sh. 3.7 për +3.7% ose -4.2 për -4.2%)",
            min_value=-100.0,
            max_value=100.0,
            value=0.0,
            step=0.1,
        ...
        if price_entry > 0:
            manual_price = price_entry * (1 + manual_percent / 100)

            st.markdown("### 📌 Rezultati i përqindjes manuale")
            st.write(f"Lëvizja: **{manual_percent:.2f}%**")
            st.write(f"Çmimi i ri (10 dec): **{manual_price:.10f}**")
    else:
